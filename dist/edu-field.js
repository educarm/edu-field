...
angular.module('edu-field.tpl').run([
  ...
  function ($templateCache) {
    ...
    $templateCache.put('directives/edu-field-date-ag-ui-tpl.html', '<div class="form-group {{options.col}}" ng-class="{\'has-error\':  $invalid, \'has-success\': !$invalid && $dirty}" style=position:relative><label for={{id}}>{{options.label}} {{options.required ? \'*\' : \'\'}}</label><div class="input-group date-group"><input class="form-control input-{{options.sizeSuffix || \'sm\'}}" ng-click=internalControl.showCalendar($event) datepicker-popup="{{options.format || \'dd-MM-yyyy\'}}" ng-model=value ng-disabled={{options.disabled}} is-open=options.showPopupCalendar datepicker-options=options.dateOptions date-disabled=options.disabled current-text=Hoy clear-text=Borrar close-text="Cerrar"> <span class=input-group-btn ng-show=options.showButtonCalendar><button type=button class="btn btn-{{options.sizeSuffix || \'sm\'}} btn-info" ng-click=internalControl.showCalendar($event)><i class="glyphicon glyphicon-calendar"></i></button></span></div><div class="help-block has-error" ng-show=$invalid style=position:absolute;top:50px><small class=error ng-show=$invalidRequired>Campo obligatorio. Introduzca una fecha</small> <small class=error ng-show=$invalidPattern>Introduzca un fecha v\xe1lida (dd-mm-aaaa)</small></div></div>');
    ...
    $templateCache.put('directives/edu-field-upload15x-tpl.html', '<div flow-prevent-drop class="form-group {{options.col}}" ng-class="{\'has-error\':  $invalid, \'has-success\': !$invalid && $dirty}" style=position:relative><label for={{id}}>{{options.label}} {{options.required ? \'*\' : \'\'}}</label><div id={{id}} ng-if=options.url><div flow-init flow-files-submitted=$flow.upload() flow-file-success="onSuccessItem( $file, $message, $flow )" flow-file-added="onAddedItem( $file, $event, $flow )" flow-file-retry="onRetryItem( $file, $flow )" flow-file-error="onErrorItem( $file, $message, $flow )"><div class=drop flow-drop ng-class=dropClass><div flow-drag-enter="style={border:\'4px solid green\'}" flow-drag-leave="style={}" ng-style=style><div class=input-group><input class="form-control input-{{options.inputSize}}" aria-describedby=basic-addon1 readonly ng-model=value autofocus ng-required={{options.required}} ng-disabled={{options.disabled}} ng-blur=onBlur() ng-focus=onFocus() ng-change=onChange()> <span class=input-group-btn flow-btn><label class="btn btn-{{options.typeButton || \'default\'}} btn-{{options.inputSize}}" type=button><i class="fa fa-{{options.iconButton}}"></i> {{options.labelButton}}</label></span></div></div></div><br><div ng-show=options.showbuttonsAll class=well><a class="btn btn-small btn-success" ng-click=$flow.resume()>Reanudar todos</a> <a class="btn btn-small btn-danger" ng-click=$flow.pause()>Pausar todos</a> <a class="btn btn-small btn-info" ng-click=$flow.cancel()>Cancelar todos</a> <span class="label label-info">Tama\xf1o: {{$flow.getSize()}} bytes</span></div><div><div ng-show=options.showprogressbar ng-repeat="file in $flow.files" class=transfer-box>{{file.relativePath}} ({{file.size}}bytes)<div class="progress progress-striped" ng-class="{active: file.isUploading()}"><div class=progress-bar role=progressbar aria-valuenow="{{file.progress() * 100}}" aria-valuemin=0 aria-valuemax=100 ng-style="{width: (file.progress() * 100) + \'%\'}"><span class=sr-only>{{file.progress()}}% Complete</span></div></div><div ng-show=options.showbuttons class=btn-group><a class="btn btn-xs btn-warning" ng-click=file.pause() ng-show="!file.paused && file.isUploading()">Pausar</a> <a class="btn btn-xs btn-warning" ng-click=file.resume() ng-show=file.paused>Reanudar</a> <a class="btn btn-xs btn-danger" ng-click=file.cancel()>Cancelar</a> <a class="btn btn-xs btn-info" ng-click=file.retry() ng-show=file.error>Reintentar</a></div></div></div></div></div></div><div class="help-block has-error" ng-show=$invalid style=position:absolute;top:50px><small class=error ng-show=$invalidRequired>Campo obligatorio. Introduzca un valor</small></div>');
