        ...
        function ($scope, $element, $attrs) {
          ...
          function fixHeaderWidths() {
            ...
            $element.find('table th .th-inner:visible').each(function (index, el) {
              ...
              var width = el.parent().width(), lastCol = $element.find('table th:visible:last'), headerWidth = width;
              ...
              var minWidth = _getScale(el.parent().css('min-width')), title = el.parent().attr('title');
          ...
          function renderChains() {
            var resizeQueue = waitForRender().then(fixHeaderWidths), customHandlers = $scope.headerResizeHanlers || [];
      ...
      controller: [
        ...
        function fieldController($scope, $http, Upload, FileUploader, dataFactoryField) {
          ...
          if ($scope.options.type == 'grid') {
            ...
            $scope.options.overlayFormSuccessErrorInputGrid = {
              show: false,
              type: 'danger',
              message: ''
            };
            ...
            $scope.gridSave = function (item) {
              ...
              if (item.$inserted) {
                ...
                apiField.update(oId, dataTemp, function (data) {
                  ...
                  $scope.options.overlayFormSuccessErrorInputGrid.message = 'No ha sido posible realizar la modificaci\xf3n: ' + data.data;
                  $scope.options.overlayFormSuccessErrorInputGrid.show = true;
                });
              }
            };
...
angular.module('edu-field.tpl').run([
  ...
  function ($templateCache) {
    ...
    $templateCache.put('directives/edu-field-date-ag-ui-tpl.html', '<div class="form-group {{options.col}}" ng-class="{\'has-error\':  $invalid, \'has-success\': !$invalid && $dirty}" style=position:relative ng-hide=options.hidden><label for={{id}}>{{options.label}} {{options.required ? \'*\' : \'\'}}</label><div class="input-group date-group"><input class="form-control input-{{options.inputSize}}" datepicker-localdate ng-click="(options.disabled || options.readonly)?angular.noop():internalControl.showCalendar($event)" datepicker-popup="{{options.format || \'dd-MM-yyyy\'}}" min-date=options.minDateObj max-date=options.maxDateObj datepicker-options=options.dateOptions ng-blur=onBlur() ng-focus=onFocus() ng-change=onChange() ng-keypress=onKeypress($event) ng-model=value ng-required=options.required ng-disabled=options.disabled ng-readonly=options.readonly is-open=options.showPopupCalendar edu-focus=options.autofocus current-text=Hoy clear-text=Borrar close-text="Cerrar"> <span class=input-group-btn ng-show=options.showButtonCalendar><button type=button ng-disabled=options.disabled class="btn btn-{{options.inputSize || \'sm\'}} btn-info" ng-click="(options.disabled || options.readonly)?angular.noop():internalControl.showCalendar($event)"><i class="glyphicon glyphicon-calendar"></i></button></span></div><div class="help-block has-error" ng-show=$invalid style=position:absolute;top:50px><small class=error ng-show=$invalidRequired>Campo obligatorio. Introduzca una fecha</small> <small class=error ng-show=$invalidPattern>Introduzca un fecha v\xe1lida (dd-mm-aaaa)</small></div></div>');
    ...
    $templateCache.put('directives/edu-field-grid-tpl.html', '<style>.input-grid table {\r' + '\n' + '\t\tborder-collapse: collapse;\r' + '\n' + '\t}\r' + '\n' + '\r' + '\n' + '\tinput-grid table, th, td {\r' + '\n' + '\t\tborder: 1px solid #DCDCDC;\r' + '\n' + '\t}</style><div class="form-group {{options.col}}" ng-class="{\'has-error\':  $invalid, \'has-success\': !$invalid && $dirty}" ng-hide=options.hidden><label for=input-grid-{{options.key}}>{{options.label}} {{options.required ? \'*\' : \'\'}}</label><div style="overflow-x:scroll; height:{{(options.height*1+40)}}px; position:relative" class=form-control id=input-grid-{{options.key}}><scrollable-table watch=gridRows><table border=1 ng-style=widthTableGrid class=input-grid><thead><tr><th width=60px title="Nuevo elemento" ng-show="options.showButtons!=false"><button ng-click=gridNew() class="btn btn-sm btn-info" ng-hide=options.buttonNewHidden ng-disabled="options.state!=\'list\' || options.buttonNewDisabled"><span class="fa fa-plus"></span></button></th><th ng-repeat="field in options.listFields" class=noFixedColumn width={{field.weight}}% title=&nbsp style="text-align: center" id={{$index}}><span ng-click="changeOrder(field, field.orderByValue, \'desc\')" ng-show="field.order==\'asc\'" class=header-column title="Ordenar por {{field.label}}"><i class="glyphicon glyphicon-sort-by-alphabet"></i> <a>&nbsp{{field.label}}</a></span> <span ng-click="changeOrder(field, field.orderByValue, \'asc\')" ng-show="field.order==\'desc\'" class=header-column title="Ordenar por {{field.label}}"><i class="glyphicon glyphicon-sort-by-alphabet-alt"></i> <a>&nbsp{{field.label}}</a></span> <span ng-click="changeOrder(field, field.orderByValue, \'desc\')" ng-hide="field.order.length>0" class=header-column title="Ordenar por {{field.label}}"><a>&nbsp{{field.label}}</a></span></th></tr></thead><tbody><tr ng-repeat="item in gridRows" ng-click=options.gridOnRowClick(item)><td style="white-space: nowrap" ng-show="options.showButtons!=false"><form ng-show=item.$visible class="form-buttons form-inline" shown=item.$inserted><button ng-click=gridSave(item) ng-disabled="item.$waiting || options.buttonSaveDisabled" class="btn btn-sm btn-warning"><span class="fa fa-save"></span></button> <button type=button ng-show="options.state!=\'new\'" ng-disabled="item.$waiting || options.buttonUndoDisabled" ng-click=gridCancel(item) class="btn btn-sm btn-default"><span class="fa fa-undo"></span></button> <button class="btn btn-sm btn-danger" ng-show="options.state==\'new\'" ng-hide=options.buttonDeleteHidden ng-disabled=options.buttonDeleteDisabled ng-click=gridDelete(item,$index)><span class="fa fa-remove"></span></button></form><div class=buttons ng-show=!item.$visible><button class="btn btn-sm btn-primary" ng-disabled=options.buttonEditDisabled ng-hide=options.buttonEditHidden ng-click=gridEdit(item)><span class="fa fa-edit"></span></button> <button class="btn btn-sm btn-danger" ng-disabled=options.buttonDeleteDisabled ng-hide=options.buttonDeleteHidden ng-click=gridDelete(item,$index)><span class="fa fa-remove"></span></button></div></td><td ng-repeat="field in options.listFields" ng-click=options.gridOnRowClick()><div ng-switch on=field.type><div ng-switch-when=autocomplete><span ng-show=!item.$visible>{{item.$optionSelectedName[field.column]}}</span> <span ng-show=item.$visible><edu-complete id_={{id}} name={{name}} onblur=onBlur() onfocus=onFocus() edu-focus=options.autofocus required placeholder={{options.placeholder}} pause={{autocpause}} selectedobject=value disabled readonly url={{options.autocurldata}} urldataloadall={{options.autocurldataloadall}} localdata=options.autoclocaldata searchfields={{options.autocsearchfields}} datafield={{options.autocfieldvalue}} titlefield={{options.autocfieldtitle}} descriptionfield={{options.autocfielddescription}} imagefield={{options.autocfieldimg}} minlength={{options.autocminlength}} inputclass="form-control input-{{options.inputSize}}" widthdropdown=options.widthdropdown whennotfoundreturnsearchstring="options.whenNotFoundReturnSearchString"></span></div><div ng-switch-when=text><span ng-show=!item.$visible>{{item[field.column]}}</span> <span ng-show=item.$visible><input style=width:100% ng-disabled=field.disabled ng-model=item[field.column] class="form-control"></span></div><div ng-switch-when=number class=pull-right style=margin-right:10px><span ng-show=!item.$visible>{{field.renderer(item[field.column], entry, field.column,field.type)}}</span> <span ng-show=item.$visible><input style=width:100% ng-disabled=field.disabled ng-model=item[field.column] class="form-control"></span></div><div ng-switch-when=date><span ng-show=!item.$visible>{{item[field.column] | date:field.format ||\'dd/MM/yyyy\'}}</span> <span ng-show=item.$visible><input style=width:100% type=date ng-disabled=field.disabled ng-model=item[field.column] class="form-control"></span></div><div ng-switch-when=currency><span ng-show=!item.$visible>{{item[field.column] | toEuros}}</span> <span ng-show=item.$visible><input style=width:100% ng-disabled=field.disabled ng-model=item[field.column] class="form-control"></span></div><div ng-switch-when=checkbox><span ng-show=!item.$visible><input type=checkbox ng-model=item[field.column] ng-false-value={{options.false_value}} ng-true-value={{options.true_value}} onclick="return false"></span> <span ng-show=item.$visible><input style=width:100% type=checkbox ng-disabled=field.disabled ng-model=item[field.column] ng-false-value="\'N\'" ng-true-value="\'S\'"></span></div><div ng-switch-when=select><span ng-show=!item.$visible>{{item.$optionSelectedName[field.column]}}</span> <span ng-show=item.$visible><select style=width:100% ng-disabled=field.disabled ng-change=onChange(item,field,field.column,item[field.column]) ng-model=item[field.column] class=form-control><option></option><option ng-repeat="option in field.selectOptions" value={{option.value}}>{{option.name}}</option></select></span></div><div ng-switch-default><span ng-show=!item.$visible>{{item[field.column]}}</span> <span ng-show=item.$visible><input style=width:100% ng-disabled=field.disabled ng-model="item[field.column]"></span></div></div></td></tr></tbody></table></scrollable-table></div><div ng-show=options.inputGridLoading class=overlay-edu-grid><div class="spin centrado-edu-grid"></div></div><div name=overlay class=overlay-edu-crud ng-show=options.showOverlayInputGridFormDelete><div class=centrado-edu-crud><div class="panel panel-info"><div class=panel-heading><div class=row><div class=col-md-12><h4>{{options.snippets.formDeleteTitle || \'Por favor confirme\'}}</h4></div></div></div><div class=panel-body><h4>{{options.snippets.formDeleteMessage || \'\xbfEst\xe1 seguro que quiere ELIMINAR el registro\'}} {{keyRowForDelete}} ?</h4></div><div class=panel-footer><div class=row><div class="col-md-offset-6 col-md-6"><button ng-click=inputGridFormDeleteContinue() class="btn btn-sm btn-primary">{{options.snippets.formDeleteButtonContinue || \'Continuar\'}}</button> <button ng-click=inputGridFormDeleteCancel() class="btn btn-sm">{{options.snippets.formDeleteButtonCancel || \'Cancelar\'}}</button></div></div></div></div></div></div><div class=overlay-edu-crud ng-show=options.overlayFormSuccessErrorInputGrid.show><div class="panel panel-{{options.overlayFormSuccessErrorInputGrid.type|| \'info\'}} centrado-edu-grid" style=min-width:{{options.overlayFormSuccessErrorInputGrid.width||200}}px><div class=panel-heading><span ng-if="options.overlayFormSuccessErrorInputGrid.type==\'success\'" class="glyphicon glyphicon-ok pull-right"></span> <span ng-if="options.overlayFormSuccessErrorInputGrid.type==\'danger\'" class="glyphicon glyphicon-remove pull-right"></span><br></div><div class=panel-body><h4>{{options.overlayFormSuccessErrorInputGrid.message}}</h4></div><div class=panel-footer><div class=row><div class="col-md-offset-3 col-md-9"><button ng-click="options.overlayFormSuccessErrorInputGrid.show=false" class="btn btn-sm btn-primary">{{options.snippets.overlayFormSuccessErrorInputGrid || \'Aceptar\'}}</button></div></div></div></div></div></div>');
    ...
    $templateCache.put('directives/edu-field-tritoggle-tpl.html', '<div class="form-group {{options.col}}" ng-class="{\'has-error\':  $invalid, \'has-success\': !$invalid && $dirty}" style=position:relative ng-hide=options.hidden><label for={{id}}>&nbsp;</label><div class="form-control input-{{options.inputSize}}" style="border:solid 0px"><div three-state-checkbox ng-model=value ng-change=onChange() options="{color: \'red\'}"></div>{{options.label}} {{options.required ? \'*\' : \'\'}}</div><div class="help-block has-error" ng-show=$invalid style=position:absolute;top:50px><small class=error ng-show=$invalidRequired>Campo obligatorio. Introduzca un valor</small></div></div>');
